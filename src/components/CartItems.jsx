            ...
            <table>
                ...
                <tbody>
                    {all_products.map((e) => {
                        if (cartItems[e.id] > 0) {
                            return (
                                <tr key={e.id} className='border-b border-slate-900/20 text-gray-30 medium-14 text-center'>
                                    <td className='flexCenter'><img src={e.img} alt='prdctImg' height={43} width={43} className='rounded-lg ring-1 ring-slate-900/5 my-1'/></td>
                                    <td><div className='line-clamp-3'>{e.name}</div></td>
                                    <td>${e.new_price}</td>
                                    <td className='w-16 h-16 bg-white'>{cartItems[e.id]}</td>
                                    <td>${e.new_price * cartItems[e.id]}</td>
                                    <td>
                                        <div className='blod-22 pl-14'><TbTrash onClick={() => removeFromCart(e.id)}/></div>
                                    </td>
                                </tr>
                            );
                        }
            ...
            <div className='flex- flex-col gap-20 my-16 p-8 md:flex-row rounded-md bg-white w-full max-w-[666px]'>
                <div className='flex flex-col gap-10'>
                    ...
                    <div>
                        ...
                        <hr/>
                        <div className='flexBetween py-4'>
                            <h4 className='blod-18'>Total:</h4>
                            <h4 className='blod-18'>${getTotalCartAmount()}</h4>
                        </div>
                    </div>
                    <button className='btn_dark_rounded w-44'>Checkout</button>
                    <div className='flex flex-col gap-10'>
                        <h4 className='blod-20 capitalize'>Your coupon code enter here:</h4>
                        <div className='flexBetween pl-5 h-12 bg-primary rounded-full ring-slate-900/10'>
