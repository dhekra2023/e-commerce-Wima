  ...
  return (
    ...
    <div>
      ...
      <div className="flexBetween my-8 mx:2">
        <h5><span>Showing 1-12</span>out of 36 
        products</h5>
        <div className="flexBetween max-sm:p-4 gap-x-4 px:8 py-3 rounded-5xl
        ring-1 ring-slate-900/15">Sort by <MdOutlineKeyboardArrowDown/>
        </div>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 
        ...
        {all_products.map((item) => {
                    if(category ===item.category){
                      return <Item key={item.id} id={item.id} image={item.img} name={item.name} 
